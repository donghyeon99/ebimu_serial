...
private:
 virtual void imuTimer_callback(const ros::TimerEvent& event) {
  ...
  virtual void pub1_odom1(vector<string> split_data)
  {
      ...
      // TODO: Use prefix for multiple drone test
      ...
      odomMsg.twist.twist.angular.z = std::stof(split_data[3]); // imu - yaw
  ...
  virtual void pub1_odom2(vector<string> split_data)
  {
      ...
      odomMsg.header.stamp = ros::Time::now();
      odomMsg.header.frame_id = "odom";
      odomMsg.child_frame_id = "base_link_robot1";
      odomMsg.twist.twist.angular.x = std::stof(split_data[1]); // imu - roll
      odomMsg.twist.twist.angular.y = std::stof(split_data[2]); // imu - pitch
      odomMsg.twist.twist.angular.z = std::stof(split_data[3]); // imu - yaw
      odomMsg.twist.twist.linear.x = std::stof(split_data[4]);  // imu - x velocity
      ...
      odomMsg.header.stamp = ros::Time::now();
      odomMsg.header.frame_id = "odom";
      odomMsg.child_frame_id = "base_link_robot2";
      odomMsg.twist.twist.angular.x = std::stof(split_data[8]); // imu - roll
      odomMsg.twist.twist.angular.y = std::stof(split_data[9]); // imu - pitch
      odomMsg.twist.twist.angular.z = std::stof(split_data[10]); // imu - yaw
      odomMsg.twist.twist.linear.x = std::stof(split_data[11]);  // imu - x velocity
      odomMsg.twist.twist.linear.y = std::stof(split_data[12]);  // imu - y velocity
      odomMsg.twist.twist.linear.z = std::stof(split_data[13]);  // imu - z velocity

      ROS_DEBUG_STREAM(odomMsg.pose.pose);
      odomMsg.pose.covariance[0] = cov_x_;
      odomMsg.pose.covariance[7] = cov_y_;
      odomMsg.pose.covariance[14] = cov_z_;
      pub2_.publish(odomMsg);
  }
  private:
    ros::NodeHandle nh_;
